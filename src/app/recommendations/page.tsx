    ...
    return (
        <div className="p-6">
            ...
            <Suspense fallback={<NewAlbumsSkeleton number={12} />}>
                <RecommendationContent page={currentPage} limit={limit} offset={offset} />
            </Suspense>
        </div>
    );
}

async function RecommendationContent({ page, limit, offset }: { page: number; limit: number; offset: number }) {
    const data = await fetchRecommendations({ limit, offset });
    return <RecommendationList page={page} recommendations={data?.albums?.items} limit={limit} />;
}
